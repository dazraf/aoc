package io.dazraf.aoc.y2020.day17

import io.dazraf.aoc.Puzzle
import io.dazraf.aoc.utilities.*

fun main() = Puzzle(2020, 17, "Conway Cubes").solve(Puzzle::part1, Puzzle::part2)

fun Puzzle.part1() = runSimulation(3, 6).cellCount
fun Puzzle.part2() = runSimulation(4, 6).cellCount

private fun Puzzle.runSimulation(dimensions: Int, iterations: Int) =
...
private fun Puzzle.readSpace(dimensionCount: Int): HyperSpace {
  check(dimensionCount >= 2) { "dimensions must be greater than 1" }
  val seed = dataSequence.map { line -> line.map { it == '#' } }.map { it.toBooleanArray() }.toTypedArray()
